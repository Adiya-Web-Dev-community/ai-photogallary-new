...
const storage = multer.diskStorage({
    destination: async function (req, file, cb) {
        ...
        if (!eventId || !category) {
            return cb(
                new Error(
                    "Event ID and category must be provided in the request body"
                )
            );
        }
...
const compressImages = async (req, res, next) => {
    try {
        if (req.files) { // Use req.files instead of req.file
            ...
            for (const f of req.files) { // Iterate through req.files
                if (f.size > 1024 * 1024 * 1.5) { // Check if the file size exceeds 1.5MB
                    // Compress the image
                    const compressedImage = await sharp(f.buffer).jpeg({ quality: 90, progressive: true }).toBuffer();
                    const compressedSize = compressedImage.length;
                    if (compressedSize <= 1024 * 1024 * 1.5) { // Ensure the compressed image is within 1.5MB
            ...
            req.files = [...req.files.filter(f => !compressedFiles.includes(f)), ...compressedFiles];
        }
        next();
    } catch (error) {
